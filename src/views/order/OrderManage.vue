<template>
  <div class="userManageContent">
      ...
      <div class="dataContent">
          ...
          <div class="searchForm">
            <el-form :inline="true" class="demo-form-inline">
              <el-form-item label="公司名:" width="400px">
                <el-input v-model="searchParams.companyName" placeholder="请输入公司名"></el-input>
              </el-form-item>
              <el-form-item label="联系人:" width="400px">
                <el-input v-model="searchParams.contacts" placeholder="请输入联系人"></el-input>
              </el-form-item>
              <el-form-item label="花号:" width="400px">
                <el-input v-model="searchParams.flowerNum" placeholder="请输入花号"></el-input>
              </el-form-item>
              <el-form-item label="面料规格:" width="400px">
                <el-input v-model="searchParams.key" placeholder="请输入面料规格"></el-input>
              </el-form-item>
              ...
              <el-form-item label="订单分类:">
                <el-select v-model="searchParams.type1">
                  <el-option v-for="(item,index) in type1List" :key="index" :label="item" :value="index"></el-option>
                </el-select>
              </el-form-item>
        ...
        <div class="dataListArea">
          <el-table row-class-name="trStyle" class="tableData" :data="tableData" border stripe width="100%">
            <el-table-column align="center" :show-overflow-tooltip="true" prop="orderId" label="订单编号">
            </el-table-column>
            <el-table-column align="center" :show-overflow-tooltip="true" prop="companyName" label="公司名">
            </el-table-column>
            <el-table-column align="center" :show-overflow-tooltip="true" prop="phone" label="联系电话">
            </el-table-column>
            ...
            <el-table-column align="center" :show-overflow-tooltip="true" prop="badMeter" label="次品米数">
            </el-table-column>
            <el-table-column align="center" :show-overflow-tooltip="true" prop="unitPrice" label="单价">
            </el-table-column>
            <el-table-column align="center" :show-overflow-tooltip="true" prop="totalPrice" label="总价">
            </el-table-column>
            <el-table-column align="center" :show-overflow-tooltip="true" prop="orderNeededTime" label="订单用时">
            </el-table-column>
            <el-table-column align="center" :show-overflow-tooltip="true" prop="fabricName" label="面料规格">
            </el-table-column>
            <el-table-column align="center" prop="meter" label="打印数量">
            </el-table-column>
            <el-table-column align="center" prop="unfinishMeter" label="未完成米数">
            </el-table-column>
            <el-table-column align="center" :show-overflow-tooltip="true" prop="companyName" label="业务员">
              <template slot-scope="scope">
                {{scope.row.salesMan.name}}
              </template>
            </el-table-column>
            <el-table-column align="center" :show-overflow-tooltip="true" label="订单状态">
              <template slot-scope="scope">
                {{scope.row.type != 100 ?typeList[scope.row.type+1] : '订单已取消'}}
              </template>
            </el-table-column>
            <el-table-column align="center" :show-overflow-tooltip="true" prop="onlinePrinter" label="在线打印员">
            </el-table-column>
            <el-table-column align="center" :show-overflow-tooltip="true" label="瑕疵扣除积分">
              <template slot-scope="scope">
                <span v-if="scope.row.type>=6">{{scope.row.reduceScoreTotal}}</span>
                <span v-else>暂未检验</span>
              </template>
            </el-table-column>
            <el-table-column align="center" :show-overflow-tooltip="true" prop="comment1" label="客户评论">
            </el-table-column>
            <el-table-column align="center" :show-overflow-tooltip="true" prop="comment2" label="业务员评论">
            </el-table-column>
            <el-table-column align="center" label="操作" width="340px" fixed="right">
              <template slot-scope="scope">
                <el-button type="primary" size="mini" @click="showDetail(scope.row)">查看详情</el-button>
                <el-button type="primary" size="mini" @click="orderAgain(scope.row)">再次下单</el-button>
                <el-button type="primary" v-if="userType == 2&&scope.row.type!=100" size="mini" @click="check(scope.row)">编辑</el-button>
                <el-button type="primary" v-if="scope.row.type==0&&userType==2"  size="mini" @click="review(scope.row)">审核</el-button>
                <el-button type="primary" v-if="scope.row.type != 100" size="mini" @click="showPrint(scope.row)">打印</el-button>
              </template>
            </el-table-column>
          </el-table>
    ...
    <div>
      <el-drawer size="50%" :title="drawerTitle" :visible.sync="drawer" :destroy-on-close="true" direction="rtl">
          ...
          <el-form>
            ...
            <el-row>
              <el-form label-width="100px">
                <el-form-item label="花型:">
                  <el-col :span="4" :offset="1" v-for="(item,index) in detailInfo.orderDetails" :key="index">
                    <img style="width: 100%;" :src="baseUrl+item.flowerUrl"/>
                    <div style="width: 100%;text-align: center;line-height: 20px;">{{item.flowerNum}}</div>
                  </el-col>
                </el-form-item>
              </el-form>
            </el-row>
            ...
            <el-row>
              ...
              <el-col :span="12">
                <el-form label-width="100px">
                  <el-form-item label="订单状态:">
                    <label>{{detailInfo.type != 100 ?typeList[detailInfo.type+1] : '订单已取消'}}</label>
                  </el-form-item>
                </el-form>
              </el-col>
              ...
              <el-col :span="12">
                <el-form label-width="100px">
                  <el-form-item label="发货方式:">
                    <label>{{detailInfo.expressType+detailInfo.expressType=='自提'?'':detailInfo.expressNumber}}</label>
                  </el-form-item>
                </el-form>
              </el-col>
            </el-row>
              ...
              <el-col :span="11" :offset="1">
                <el-form label-width="100px">
                  <el-form-item label="备注:">
                    ...
                    <div v-if="detailInfo.note2">厂长备注：{{detailInfo.note2}}</div>
                    <div v-if="detailInfo.type==8">退货备注：{{detailInfo.note3}}</div>
                  </el-form-item>
                </el-form>
              </el-col>
            </el-row>
              ...
              <el-col :span="11" :offset="1">
                <el-form label-width="100px">
                  <el-form-item label="瑕疵积分扣除:">
                    <label v-if="detailInfo.type>=6">{{-detailInfo.reduceScoreTotal}}{{detailInfo.meter/100*20>=detailInfo.reduceScoreTotal?"（合格）":"（不合格）"}}</label>
              ...
              <el-col :span="22" :offset="1">
                <el-form label-width="100px">
                  <el-form-item label="客户评论:">
                    ...
                    <br/>
                    <el-image v-for="item in detailInfo.orderCommentList0"
                      :key="item.url"
              ...
              <el-col :span="22" :offset="1">
                <el-form label-width="100px">
                  <el-form-item label="操作记录:">
                    <el-table row-class-name="trStyle" class="tableData" :data="detailInfo.orderOperations" border stripe width="100%">
                      <el-table-column align="center" :show-overflow-tooltip="true" prop="fkUserId" label="操作人编号" >
                      </el-table-column>
                      <el-table-column align="center" :show-overflow-tooltip="true" prop="userName" label="操作人">
                      </el-table-column>
                      <el-table-column align="center" prop="type" label="操作类型">
                        <template slot-scope="scope">
                          {{scope.row.type != 0 ? operateTypeList[scope.row.type] : scope.row.note ? scope.row.note : operateTypeList[scope.row.type]}}
                          {{scope.row.type==4?'('+scope.row.meter+'米)':''}}
                        </template>
                      </el-table-column>
                    </el-table>
                  </el-form-item>
              ...
              <el-col :span="22" :offset="1">
                <el-form label-width="100px">
                  <el-form-item label="瑕疵记录:">
                    <el-table row-class-name="trStyle" class="tableData" :data="detailInfo.orderFlaws" border
                              stripe width="100%">
                      <el-table-column align="center" :show-overflow-tooltip="true" prop="flawName" label="瑕疵名称">
                      </el-table-column>
                      <el-table-column align="center" :show-overflow-tooltip="true" prop="reduceScore" label="扣除分数">
                      </el-table-column>
                      <el-table-column align="center" prop="type" label="瑕疵位置">
                        <template slot-scope="scope">
                          第{{scope.row.num}}米
                        </template>
                      </el-table-column>
            ...
            <el-row v-if="operateType=='update'">
              <el-col :span="11" :offset="1">
                <el-form-item label="厂长备注:">
                  <el-input type="textarea" v-model="detailInfo.note2"></el-input>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
              <el-col :span="11" :offset="1">
                <el-form label-width="100px">
                  <el-form-item>
                    <el-button  v-if="operateType=='update'" type="primary" @click="add">保存</el-button>
    ...
    <!--打印表格-->
    <!--订单打印-->
    <div class="orderPrintDialog" id="orderPrintDialog" v-if="orderPrintDialogVisible==true">
      <el-dialog
        title=""
        :visible.sync="orderPrintDialogVisible"
        fullscreen>
        <el-row>
          <el-col :span="22" :offset="1">
            <div id="printArea">
              <table cellspacing="0" cellpadding="0" style="font-size: 16px;border: 1px solid #000;overflow: hidden;border-collapse: collapse;margin: 0 auto;width: 90%;">
                ...
                <tr>
                  <th colspan="8">
                    <div style="font-size: 20px;text-align: center;letter-spacing: 2px;line-height: 160%;">华燕数码工艺流程单{{detailInfo.orderId}}</div>
                  </th>
                </tr>
                </thead>
                <tbody>
                  ...
                  <tr>
                    <td colspan="8">
                      <div class="memo" style="width: 100%;">
                        <span style="font-size: 16px;">备注：{{detailInfo.note}}</span>
                      </div>
                      <div class="imgBox" style="display: flex;width: 100%;align-items: center;justify-content: center;margin-top: 50px;margin-bottom: 50px;">
                        <div style="width: 40%;margin-right: 1%;" v-for="(item,index) in detailInfo.orderDetails" :key="index">
                          <img  style="width: 100%;" :src="baseUrl+item.flowerUrl"/>
                          <div style="font-size: 16px;width: 100%;text-align: center;overflow: hidden;white-space: nowrap;text-overflow: ellipsis;">{{item.flowerNum}}</div>
                        </div>
                      </div>
                    </td>
                  </tr>
                  ...
                  <tr>
                    ...
                    <td width="12%">审批：</td>
                    <td colspan="3"  width="30%">{{detailOperateInfo.reseverName}}</td>
                  </tr>
                  <tr>
                    <td width="12%">领面料：</td>
                    ...
                    <td width="12%">米数：</td>
                    <td colspan="3"  width="30%">{{detailOperateInfo.getFUser?detailInfo.meter+'米':''}}</td>
                  </tr>
                  <tr v-for="(item,index) in detailOperateInfo.printList" :key="index+'print'">
                    <td width="12%">打印：</td>
                    <td width="15%">{{item.printUser}}</td>
                    <td width="12%">时间：</td>
                    <td width="19%">{{item.printTime}}</td>
                    <td width="12%">米数：</td>
                    <td colspan="3" width="30%">{{item.printMeter?item.printMeter+'米':''}}</td>
                  </tr>
                  <tr>
                    <td width="12%">蒸花：</td>
                    <td width="15%">{{detailOperateInfo.flowerUser}}</td>
                    <td width="12%">时间：</td>
                    <td colspan="5" width="61%">{{detailOperateInfo.flowerTime}}</td>
                  </tr>
                  <tr>
                    <td width="12%">出库：</td>
                    ...
                    <td width="12%">出库米数：</td>
                    <td width="9%">{{detailInfo.okMeter?detailInfo.okMeter+'米':''}}</td>
                    <td width="12%">次品米数：</td>
                    <td width="9%">{{detailInfo.badMeter?detailInfo.badMeter+'米':''}}</td>
                  </tr>
                  <tr>
                    <td colspan="8" style="position: relative">
                      <div class="memo" style="width: 100%;padding-bottom: 100px;">
                        <span style="font-size: 16px;">备注：{{detailInfo.note2}}</span>
                        <vue-qr v-if="qrCodeUrl != ''" style="width: 100px;height: 100px;position: absolute;bottom: 10px;right: 10px;" :text="qrCodeUrl" :size="200"></vue-qr>
                        <div v-if="detailInfo.note3" style="font-size: 16px;">退货备注：{{detailInfo.note3}}</div>
                      </div>
                    </td>
                  </tr>
                </tbody>
            ...
            <el-row>
              <div style="margin:20px auto;width: 200px">
                <el-button type="success" style="cursor: pointer"  v-print="'#printArea'">打印</el-button>
              </div>
            </el-row>
          </el-col>
        </el-row>
